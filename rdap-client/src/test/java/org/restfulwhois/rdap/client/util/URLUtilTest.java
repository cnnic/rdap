...
public class URLUtilTest{
	...
	public void test_makeURLWithPath(){
		String urlString;
		try{
			urlString = URLUtil.makeURLWithPath(url, "ip", "192.168.1.1").toString();
		}catch(RdapClientException e){
			urlString = null;
		}
	...
	public void test_makeURLWithPathAndParam(){
		...
		try{
			...
			map.put("p1", "1");
			map.put("p2", "2");
			urlString = URLUtil.makeURLWithPathAndParam(url, map, "ip", "192.168.1.1").toString();
		}catch(RdapClientException e){
			urlString = null;
		}
	...
	public void test_makeURLWithPathAndParam_list(){
		...
		try{
			...
			list.add("ip");
			list.add("192.168.1.1");
			urlString = URLUtil.makeURLWithPathAndParam(url, map, list).toString();
		}catch(RdapClientException e){
			urlString = null;
		}
